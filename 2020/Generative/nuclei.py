...
def bounded(seed, limits, probabilities, generations):
    seed[boundy1:boundy2, boundx1, :] = b
    seed[boundy1:boundy2, boundx2, :] = b
    seed[boundy1, boundx1:boundx2, :] = b
    seed[boundy2, boundx1:boundx2, :] = b
    tic = time.time()
    f = plt.figure()
    simulation = []
    simulation.append([plt.imshow(seed)])

    rlim = limits[0]; rhit = probabilities[0]*100
    glim = limits[1]; ghit = probabilities[1]*100
    blim = limits[2]; bhit = probabilities[2]*100
    ...
    gact = limits[1]
    bact = limits[2]
    dims = [seed.shape[0], seed.shape[1]]
    ind2sub = imutils.ind2sub(dims)
    for generation in tqdm(range(generations)):
        rch = ndi.convolve(seed[:, :, 0], k1, origin=0)
        gch = ndi.convolve(seed[:, :, 1], k1, origin=0)
        bch = ndi.convolve(seed[:, :, 2], k1, origin=0)
